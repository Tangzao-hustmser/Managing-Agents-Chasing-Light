...
def get_current_user(
    ...
    """Read the current user from a simplified bearer token."""
    if not authorization:
        raise HTTPException(status_code=401, detail="Missing authorization header")

    parts = authorization.split()
    if len(parts) != 2 or parts[0] != "Bearer":
        raise HTTPException(status_code=401, detail="Authorization must be 'Bearer {user_id}'")
...
def login(payload: UserLogin, db: Session = Depends(get_db)):
    """Login and return a simplified bearer token."""
    try:
        user = login_user(db, payload.username, payload.password)
        user_out = UserOut.model_validate(user)
        return LoginOut(token=f"Bearer {user.id}", user=user_out)
    except ValueError as exc:
